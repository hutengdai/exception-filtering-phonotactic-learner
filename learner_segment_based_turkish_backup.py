...
class Phonotactics:
	...
	def evaluate_fscore(self, filepath):
		...
		# data.columns = ['word', 'judgment'] + [None] * (len(data.columns) - 3) + ['score']
	...
	def process_and_plot_zimmer(self, input_path):
		# Read data from file
		data = pd.read_csv(input_path, sep='\t')

		# Assign the new column names to the DataFrame
		data.columns = ['form', 'judgment', 'grammaticality', 'experiment', 'score']

		data['judgment'] = data['judgment'].astype(float)
		data['score'] = data['score'].astype(float)
		# data['zscore'] = data.groupby('subj')['judgment'].transform(zscore)
		# data['boolean'] = np.where(data['zscore'] >= 0, 1, 0)
		print(data)
		data = data.groupby("form").agg({
		# "score": "mean",
		...
		pearsoncorr, p = pearsonr(data['score'], data['judgment'])
		print('Pearson correlation: %.3f' % pearsoncorr)
		print('Pearson  pvalue: %.3f' % p)

		# 
		spearmancorr, s = spearmanr(data['score'], data['judgment'])
		print('Spearman correlation: %.3f' % spearmancorr)
		print('Spearman pvalue: %.3f' % s)

		kendalltaucorr, k = kendalltau(data['score'], data['judgment'])
		...
		gamma = goodman_kruskals_gamma(data['score'], data['judgment'])
		if gamma == "NaN":
			print('Goodman-Kruskal gamma: NaN')
		else:
			print('Goodman-Kruskal gamma: %.3f' % gamma)
	...
	def main(self,TrainingFile,FeatureFile,JudgementFile,TestingFile,MatrixFile):
		...
		# self.scan_and_judge(TestingFile, JudgementFile, pos_grammar,neg_grammar)

		self.scan_and_judge_categorical(TestingFile, JudgementFile, neg_grammar, pos_grammar)
		...
		# the dataset give you a distribution, we can sample the judgement 
		...
		# self.scan_and_judge_gradient(TestingFile, JudgementFile)
...
if __name__ == '__main__':
	...
	if language == 'baatonum':
		...
		# JudgementFile = "result/english/EnglishJudgement_%s.txt" % str(datetime.datetime.now()).split(".")[0].replace(" ", "-").replace(":","-")
	...
	if language == 'toy':
		...
		JudgementFile = "result/toy/ToyJudgement_%s.txt" % str(datetime.datetime.now()).split(".")[0].replace(" ", "-").replace(":","-")
	...
	if language == 'polish':
		...
		JudgementFile = "result/polish/polishJudgement_%s.txt" % str(datetime.datetime.now()).split(".")[0].replace(" ", "-").replace(":","-")
		humanJudgement = "NaN"
	...
	if language == 'turkish':
		...
		TestingFile = 'data/turkish/TurkishTestingData-Zimmer.txt'
		JudgementFile = "result/turkish/TurkishJudgement_%s.txt" % str(datetime.datetime.now()).split(".")[0].replace(" ", "-").replace(":","-")
		humanJudgement = "NaN"
	...
	# phonotactics.main(TrainingFile, JudgementFile, TestingFile)
	phonotactics = Phonotactics()
	phonotactics.structure = 'nonlocal'
	phonotactics.filter = True #0.8571428571428571 0.9333333333333333
	phonotactics.padding = False
	phonotactics.confidence = 0.975
	phonotactics.penalty_weight = 3 # 3 for best Fscore
	phonotactics.threshold = 0.5 # 0.5  for best Fscore
	phonotactics.model = 'filtering' #filtering

	JudgementFile = ("result/%s/judgement_%s_%s_flt-%s_pad-%s_conf-%s_pen-%s_thr-%s.txt" % 
		(
	...
	MatrixFile = ("result/%s/matrix_%s_%s_flt-%s_pad-%s_conf-%s_pen-%s_thr-%s.txt" % 
		(
		phonotactics.language,
		phonotactics.model,
		phonotactics.structure, 
	...
	def run():
		...
		phonotactics.main(TrainingFile, FeatureFile, JudgementFile, TestingFile, MatrixFile)

		if language == 'english':
			pearsoncorr, spearmancorr, kendalltaucorr, fscore = phonotactics.visual_judgefile(humanJudgement, JudgementFile)

		if language == 'polish':
			# Best Spearman correlation:  0.5989787903872147
			# Best hyperparameters:  [[0.1, 0.99]]
			input_path = JudgementFile
		...
		if language == 'turkish':
			if "zimmer" in TestingFile.lower():
				_, _, kendalltaucorr = phonotactics.process_and_plot_zimmer(JudgementFile)
			else:
				fscore = phonotactics.evaluate_fscore(JudgementFile)

	def hyperparameter_tuning(phonotactics, TrainingFile, JudgementFile, TestingFile, MatrixFile, humanJudgement, language):
		...
		for threshold in thresholds:
			...
			if language == 'english':
				_, _, kendalltaucorr, fscore = phonotactics.visual_judgefile(humanJudgement, JudgementFile)
				all_objectives.append(kendalltaucorr)  # Store fscore in the list
				...
				if "zimmer" in TestingFile.lower():
					_, _, kendalltaucorr = phonotactics.process_and_plot_zimmer(JudgementFile)
					all_objectives.append(kendalltaucorr)
					if kendalltaucorr > best_objective:
						best_objective = kendalltaucorr
						best_params = [threshold]
				else:
				...
				output_path = "result/polish/correlation_plot.png"
				output_path_ucla = "result/polish/correlation_plot_ucla.png"
				_, _, kendalltaucorr, _ = process_and_plot(JudgementFile, output_path, output_path_ucla)
	...
	run()
	# hyperparameter_tuning(phonotactics, TrainingFile, JudgementFile, TestingFile, MatrixFile, humanJudgement, language)
